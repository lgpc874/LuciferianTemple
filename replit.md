...
- June 16, 2025. CSS grimórios totalmente sincronizado - implementado reset forçado (all: unset) seguido de reaplicação específica no SmartGrimoireReader, garantindo formatação idêntica entre painel admin (.prose) e leitor principal (.grimoire-content), incluindo suporte completo para elementos span com classes (span.destaque, span.grimorio-titulo, span.grimorio-subtitulo)
```
